      ...
      <div className="border-b border-[var(--border-subtle)] bg-[var(--bg-secondary)] px-6 py-14">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mt-4 flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
              <h1 className="text-4xl font-serif font-black text-[var(--text-primary)]">My Drafts</h1>
              <p className="text-[var(--text-muted)] font-medium mt-2">Your saved bail application drafts — edit, copy, or download anytime.</p>
            </div>
            <button
              onClick={() => navigate('/my-cases')}
              className="h-11 px-6 border border-[var(--border-primary)] text-[var(--text-primary)] text-sm font-black rounded-lg flex items-center gap-2 hover:border-[var(--btn-primary-bg)] transition-all shrink-0"
            >
      ...
      <div className="max-w-6xl mx-auto px-6 py-12">
        {loading ? (
          <div className="flex flex-col items-center justify-center py-24">
            ...
            <p className="text-[var(--text-muted)] font-medium">Loading your drafts…</p>
          </div>
        ) : !selectedDraft ? (
          /* Drafts List */
          drafts.length === 0 ? (
            <div className="bg-[var(--bg-secondary)] rounded-2xl p-20 text-center border-2 border-dashed border-[var(--border-primary)]">
              ...
              <h3 className="text-xl font-black text-[var(--text-primary)] mb-2">No saved drafts yet</h3>
            ...
            <div className="space-y-4">
              {drafts.map((draft, i) => (
                <motion.div
                  ...
                  className="group bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-subtle)] shadow-sm hover:shadow-md hover:border-[var(--border-primary)] transition-all"
                >
                  <div className="px-6 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="font-black text-[var(--text-primary)]">{draft.case_title}</h3>
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      <button
                        ...
                        className="h-10 px-6 bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] text-xs font-black uppercase tracking-widest rounded-lg hover:bg-[var(--btn-primary-hover)] transition-all flex items-center gap-1.5"
                      >
          ...
          >
            {/* Toolbar */}
            <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-subtle)] shadow-sm px-6 py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-black text-[var(--text-primary)] text-sm">{selectedDraft.case_title}</h3>
              </div>
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  onClick={handleCopy}
                  className="flex-1 sm:flex-none h-9 px-4 border border-[var(--border-primary)] text-[var(--text-primary)] text-xs font-black rounded-lg flex items-center justify-center gap-1.5 hover:border-[var(--btn-primary-bg)] transition-all"
                >
                ...
                <button
                  ...
                  className="flex-1 sm:flex-none h-9 px-4 bg-[#C9A84C] text-[var(--text-primary)] text-xs font-black rounded-lg flex items-center justify-center gap-1.5 hover:bg-[#b09341] transition-all"
                >
                ...
                <button
                  ...
                  disabled={saving}
                  className="flex-1 sm:flex-none h-9 px-5 bg-[var(--btn-primary-bg)] text-[var(--btn-primary-text)] text-xs font-black rounded-lg flex items-center justify-center gap-1.5 hover:bg-[var(--btn-primary-hover)] transition-all disabled:opacity-50"
                >
            ...
            {/* Editor */}
            <div className="bg-[var(--bg-secondary)] rounded-xl border border-[var(--border-subtle)] shadow-sm overflow-hidden">
              <div className="bg-[var(--bg-surface)] px-6 py-3 border-b border-[var(--border-subtle)] flex items-center justify-between">
                <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text-muted)]">Document Editor</span>
                <Edit3 size={14} className="text-slate-300" />
              </div>
              <div className="p-6 flex justify-center bg-[var(--bg-surface)]">
                <div className="w-full max-w-3xl bg-[var(--bg-secondary)] shadow-sm rounded-lg">
                  <textarea
                    value={editedText}
                    onChange={(e) => setEditedText(e.target.value)}
                    className="w-full h-[700px] p-12 text-[var(--text-primary)] focus:outline-none resize-y rounded-lg"
